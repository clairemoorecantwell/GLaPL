...
# front rounded vowels: '[yYcC]'
# back vowels: '[uUoOaA]'
# front unrounded vowels: '[iIeE]'
# consonants: '[pbtdKGkgfvszSZHJmnNlrjTh]'
def agreeBackLocal(SR):
    indices = []
    vs = []
    vSum = 0
    for m in re.finditer('(?=([uUoOaA][_pbtdKGkgfvszSZHJmnNlrjTh]*[iIeEyYcC]))',SR):
        start = m.start(0)
        end = start + len(m.group(1))-1
        indices.append((start,end))
        vs.append(1)
...
agreeBackLocal = constraint("agreeBackLocal",agreeBackLocal,MF='M')

#beA
#beB


constraints = [Ident,agreeBackLocal]
